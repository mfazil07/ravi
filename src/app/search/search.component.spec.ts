  ...
  describe('GetUnmappedWeatherEvents', () => {
    ...
    it('should include optional parameters in the request when provided', () => {
      ...
      service.GetUnmappedWeatherEvents(
        mockClaimantId, mockCaseId, mockStartDate, mockEndDate, mockLocation, mockCountries, mockStates, mockMappedEventsOnly
      ).subscribe(response => {
        expect(response).toEqual(mockResponse);
      });
      ...
      expect(req.request.body.countries.map((c: KeyValueObject) => c.key)).toEqual(mockCountries.map((c: KeyValueObject) => c.key));
      expect(req.request.body.states.map((s: KeyValueObject) => s.key)).toEqual(mockStates.map((s: KeyValueObject) => s.key));
